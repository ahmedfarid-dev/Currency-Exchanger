...
export class DetailsComponent implements OnInit {
  ...
  @ViewChild("historicalRatesChart", { static: false }) historicalRatesChart: ChartComponent | undefined;
  ...
  updateHistoricalRatesChart(){
    ...
    for (let i=0; i<=11; i++) {
      ...
      d.setDate(0);
      historicalMonthsRates.push({month:`${monthNames[d.getMonth()]}-${d.getFullYear()}`,date:d.toISOString().split('T')[0],rate : 0});
    }

    let observableBatch: Observable<any>[] = [];

    historicalMonthsRates.forEach((historicalMonthsRate) => {
      observableBatch.push(this.currencyExchangeService.getHistoricalRate(
    ...
    forkJoin(observableBatch).subscribe((results: any[]) => {
      historicalMonthsRates.forEach((historicalMonthsRate, i) => {
        historicalMonthsRate.rate = results[i].rates[this.currencyExchangeState.toCurrency.code];
      });
